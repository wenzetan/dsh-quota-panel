...
- **宿主半**（`lib/index.js`）：注册一条仅限回环的 Connection RPC 通道
  ...
  `Authorization: Bearer <key>` 代理访问，逐行捕获错误。
...
- **预警阈值** —— 逐提供方覆盖：余额型为「预警 ¥」（自动推得 critical = 值/2），
  用量型为「预警 %」（error 自动取 max(配置值, 预警+1)）；留空恢复配置默认；
- **恢复默认** —— 一键清空全部本地设置。
...
`critical <= warn <= healthy`）由宿主半在挂载时校验。
...
- API Key 仅由宿主半通过 `ctx.credentials` 解析，只用于宿主侧到提供方的请求；
  ...
  （标签/格式/阈值），不含 credential 与 endpoint。
- 卡片只使用 `createElement`/`textContent` 构建 DOM，API 返回值绝不经过
  `innerHTML`；技术错误（401、超时、凭据缺失）只写入 `title` 悬停提示或行内错误文案，
  单行失败不影响其他行。
...
# 双面检查：宿主半 RPC 契约 + 浏览器半槽位注册/设置面板表面
