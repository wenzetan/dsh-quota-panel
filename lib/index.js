...
function validateProxies(proxies) {
	...
	for (const [name, url] of Object.entries(proxies || {})) {
		...
		try {
			...
			throw new Error(`quota-panel: config.proxies.${name} "${url}" is not a valid URL`);
		}
		if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
			throw new Error(`quota-panel: config.proxies.${name} must be http:// or https:// (socks is unsupported)`);
		}
		out[name] = url;
	}
	return out;
...
function rowSpec(provider) {
	...
	const spec = { id: provider.id, label: provider.label, kind: meta.kind };
 ...
 * two dashboard requests against the configured base URL.
 */
async function fetchRow(ctx, provider, proxies) {
	try {
		...
		const proxyUrl = provider.proxy !== undefined ? proxies[provider.proxy] : undefined;
	...
	ctx.connection.rpc.handle(RPC_CHANNEL, async (endpoint, _payload, _signal) => {
		try {
			...
			if (endpoint === 'fetch-all') {
				return { ok: true, value: { rows: await Promise.all(rows.map((provider) => fetchRow(ctx, provider, proxies))), fetchedAt: Date.now() } };
			}
