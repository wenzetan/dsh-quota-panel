...
function buildPageScript(rows, refreshMs) {
  ...
  // Collapsed capsule: worst status across providers + joined summaries.
  // Balance rows read "¥58.36"; usage rows get a "用量" hint prefix.
  function renderCapsule() {
    var order = { loading: 0, ok: 1, warn: 2, error: 3 };
    var worst = 'loading';
    var parts = [];
    for (var i = 0; i < ROWS.length; i++) {
      var s = STATE[ROWS[i].id];
      if (!s) { parts.push('…'); continue; }
      if (order[s.status] > order[worst]) worst = s.status;
      parts.push(ROWS[i].format === 'opencode-usage' ? '用量 ' + s.summary : s.summary);
    }
    capsuleEl.classList.remove('state-loading', 'state-ok', 'state-warn', 'state-error');
    capsuleEl.classList.add('state-' + worst);
    capsuleValue.textContent = parts.join(' · ');
  }
  ...
  function injectStyles() {
    var css = [
      ...
      '#dsh-quota-capsule .dsh-status-dot{flex:none;width:8px;height:8px;margin-top:0;border-radius:50%;background:var(--dsw-static-green-500,#22c55e)}',
      '#dsh-quota-panel .state-loading .dsh-status-dot{background:var(--dsw-static-neutral-bluish-400,#adb2b8)}',
      '#dsh-quota-panel .state-warn .dsh-status-dot{background:var(--dsw-static-amber-500,#f59e0b)}',
      '#dsh-quota-panel .state-error .dsh-status-dot{background:var(--dsw-static-red-500,#ef4444)}',
      '#dsh-quota-capsule .dsh-capsule-label{color:var(--dsw-alias-label-secondary,#61666b);flex:none}',
      '#dsh-quota-capsule .dsh-capsule-value{font-variant-numeric:tabular-nums;white-space:nowrap}',
  ...
  function createPanel() {
    ...
    // Collapsed capsule.
    capsuleEl = el('button', 'dsh-quota-capsule state-loading');
    ...
    capsuleEl.appendChild(el('span', 'dsh-status-dot'));
    capsuleEl.appendChild(el('span', 'dsh-capsule-label', '额度'));
    capsuleValue = el('span', 'dsh-capsule-value', '…');
    capsuleEl.appendChild(capsuleValue);
