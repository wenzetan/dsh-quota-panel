...
const FORMATS = {
    ...
    'zai-coding-quota': (body) => {
        ...
        const windows = {};
        if (tokens.length > 0)
            windows.rolling = { percent: pct(tokens[0]), resetsAt: resets(tokens[0]) };
        if (tokens.length > 1)
            windows.weekly = { percent: pct(tokens[tokens.length - 1]), resetsAt: resets(tokens[tokens.length - 1]) };
        if (time)
            windows.monthly = { percent: countPct(time), resetsAt: resets(time) };
        if (Object.keys(windows).length === 0)
            throw new Error('no TOKENS_LIMIT / TIME_LIMIT entries');
    ...
    'kimi-coding-usage': (body) => {
        ...
        if (!(limit > 0) || !Number.isFinite(used))
            throw new Error('missing usage.limit / usage.used');
        const windows = { weekly: { percent: Math.min(100, Math.round((used / limit) * 100)), resetsAt: usage?.resetTime } };
        ...
        if (sLimit > 0 && Number.isFinite(sUsed))
            windows.rolling = { percent: Math.min(100, Math.round((sUsed / sLimit) * 100)), resetsAt: detail?.resetTime };
