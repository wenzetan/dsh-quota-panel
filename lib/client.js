...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function readSettings() {
			var base = { hidden: {}, refreshMs: null, warn: {} };
			try {
		...
		/** Row view model: status + texts for one provider spec + its data. */
		function rowView(spec, entry, warnOverride) {
			var isUsage = spec.format === "opencode-usage";
			if (!entry || entry.error) {
				return {
					isUsage: isUsage, status: "error", summary: "—",
					value: "—", sub: isUsage ? "暂时无法获取用量" : "暂时无法获取余额",
					usageText: isUsage ? "暂时无法获取用量" : null, barPercent: 0, caption: "",
			...
			if (isUsage) {
				...
				if (!u || !u.rolling || !u.weekly || !u.monthly) {
					return {
						isUsage: true, status: "error", summary: "—", value: "—",
						usageText: "暂时无法获取用量", barPercent: 0, caption: "",
						title: data && data.error ? String(data.error) : "usage unavailable"
					};
				}
				...
				return {
					isUsage: true, status: status, summary: high + "%", value: null,
					usageText: labels.rolling + " " + rp + "% · " + labels.weekly + " " + wp + "% · " + labels.monthly + " " + mp + "%",
					barPercent: Math.min(Math.max(high, 0), 100),
					caption: "当前最高占用 " + high + "%",
					title: "rolling: " + rp + "%（重置于 " + fmtReset(u.rolling.resetsAt) + "）\nweekly: " + wp + "%（重置于 " + fmtReset(u.weekly.resetsAt) + "）\nmonthly: " + mp + "%（重置于 " + fmtReset(u.monthly.resetsAt) + "）"
				};
			}
			var info = data && data.balance_infos && data.balance_infos[0];
			if (!data || data.error || !info) {
				return {
					isUsage: false, status: "error", summary: "—", value: "—",
					sub: "暂时无法获取余额", usageText: null, barPercent: 0, caption: "",
					title: data && data.error ? String(data.error) : "no balance info"
				};
			}
			...
			if (!Number.isFinite(total)) {
				return {
					...
					title: "non-numeric total_balance"
				};
			}
			var tiers = effectiveTiers(spec, warnOverride);
			var status2, sub;
			if (total <= tiers.critical) { status2 = "error"; sub = "建议充值"; }
			else if (total <= tiers.warn) { status2 = "warn"; sub = "余额紧张"; }
			else if (total <= tiers.healthy) { status2 = "ok"; sub = "余额正常"; }
			else { status2 = "ok"; sub = "余额充足"; }
			return {
				isUsage: false, status: status2, summary: "¥" + total.toFixed(2),
				value: "¥" + total.toFixed(2), sub: sub, usageText: null, barPercent: 0, caption: "",
		...
		function SettingsPanel(props) {
			...
			var thresholdRows = specs.rows.map(function (spec) {
				...
				var placeholder = isUsage
					...
					: "预警 ¥（默认 " + ((spec.balanceTiers && spec.balanceTiers.warn) || 20) + "）";
				var raw = settings.warn[spec.id];
				return React.createElement("div", { key: spec.id, className: "dsh-setting-row" },
					React.createElement("span", { className: "dsh-setting-name" }, spec.label + (isUsage ? "（%）" : "（¥）")),
					React.createElement("input", {
			...
			return React.createElement("div", { className: "dsh-quota-settings" },
				...
				React.createElement("div", { className: "dsh-setting-section" },
					...
					thresholdRows.length ? thresholdRows : null),
			...
			function QuotaPanel() {
					...
					if (specs === null && loadError !== null) {
						...
						for (var j = 0; j < rows.length; j++) {
							...
							pairs.push(React.createElement("span", {
								key: rspec.id + "-value",
								className: "dsh-capsule-item state-" + view.status + (view.isUsage ? " dsh-usage" : "")
							}, view.summary));
						}
					}
				...
				if (settingsOpen) {
					bodyChildren.push(React.createElement(SettingsPanel, {
						...
						onReset: function () { updateSettings({ hidden: {}, refreshMs: null, warn: {} }); }
					}));
